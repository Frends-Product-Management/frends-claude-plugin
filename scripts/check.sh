...
node -e 'const F=require("./frends/hooks/formats.json");const re=()=>new RegExp(F.terminalStateRe,"m");const yes=["terminal state: success","\u0060terminal state: success\u0060","**terminal state: success**","- terminal state: success","- **terminal state: success**","1. terminal state: success","## terminal state: success","_terminal state: success_","__terminal state: blocked__"];const no=["> terminal state: success","I cannot claim terminal state: success yet","terminal state: successful build"];for(const s of yes){if(!re().test(s)){console.error("should match: "+s);process.exit(1)}}for(const s of no){if(re().test(s)){console.error("should not match: "+s);process.exit(1)}}' || { echo "  a state-line rendering regressed"; fail=1; }
