...
- Add three gate hooks driven by one `formats.json`. A PostToolUse recorder appends every draft mutation and validation to the open run record, names and counts only, never values. A Stop gate blocks, once, a `terminal state: success` whose record shows the last validation before the last change, closes finished runs and writes the ledger line, marking a close it could not vouch for as `unverified`. An auto-layout after the last validation is recorded but does not reopen the order check, because layout changes coordinates only. A SubagentStop gate bounces, once, a builder report or reviewer verdict missing its required sections. The recorder and the Stop gate are no-ops without an open run; the verdict gate follows the two agents wherever they run, because their report contracts are unconditional. None blocks twice, all fail open on their own errors and say so on stderr, and `scripts/check.sh` proves each of the three fires, passes and fails open from checked-in fixtures, including a corrupted `formats.json` silencing all three.
