...
When the draft was built from a plan, offer a review of the draft against that plan before anyone promotes it. Validation proves the draft compiles; it says nothing about whether the draft does what was asked. When the plan carries acceptance criteria or several Processes, offer the `/frends:build-loop` skill instead of a single pass: it keeps a run record, freezes the criteria and puts an independent review before success.
